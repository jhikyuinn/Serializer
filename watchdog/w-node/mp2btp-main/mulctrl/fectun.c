...
{ 
    printf("Usage: ./fectun [OPTION] -a [Address] -s [Address] ... -d [Address] ...\n" 
		   ...
		   "\t -a [Address]: Address for receiving packet from upper layer \n"
		   "\t -s [Address]: Source address of tunnel side \n"
		   "\t -d [Address]: Destination address of tunnel side \n"
...
void *tunnel_recv_thread(void *arg)
{
	...
	while (1) 
	{
		...
		if (packet_len == -1)
			error_handling("No activity! (Timeout)");

		if (packet_seq % 100000 == 0)
						...
						uint32_t start_seq, uint32_t end_seq, char *data, uint32_t len)
{
	...
	if (0) // (raptor_pkt->packet_seq % 1000 == 0) 
	{
