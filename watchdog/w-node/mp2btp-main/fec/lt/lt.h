...
char *lt_encode(char *source, int num_source, int num_encoded, int sym_size, int seed)
{
    ...
    else 
    {
        for (int i = 0; i < num_encoded; i++)
        {
            for (int j = 0; j < degree[i]; j++)
            {
                int idx = matrix[i][j] * sym_size;
                int enc_idx = i * sym_size;
                for (int k = 0; k < sym_size; k += 16)
                {
                    // Load 16 bytes from encoded and source
                    __m128i enc_vec = _mm_loadu_si128((__m128i*)&encoded[enc_idx + k]);
                    __m128i src_vec = _mm_loadu_si128((__m128i*)&source[idx + k]);

                    // XOR the vectors
                    __m128i result = _mm_xor_si128(enc_vec, src_vec);

                    // Store the result back to encoded
                    _mm_storeu_si128((__m128i*)&encoded[enc_idx + k], result);
                }
                // for (int k = 0; k < sym_size; k += 64) // 64바이트 단위로 처리
                // {
                //     // Load 64 bytes from encoded and source
                //     __m512i enc_vec = _mm512_loadu_si512((__m512i*)&encoded[enc_idx + k]);
                //     __m512i src_vec = _mm512_loadu_si512((__m512i*)&source[idx + k]);

                //     // XOR the vectors
                //     __m512i result = _mm512_xor_si512(enc_vec, src_vec);

                //     // Store the result back to encoded
                //     _mm512_storeu_si512((__m512i*)&encoded[enc_idx + k], result);
                // }
            }
        }
    }
...
char *lt_decode(char *encoded, int num_encoded, int num_source, int sym_size, char *non_error, int seed)
{
    ...
    else 
    {
        for (int k = 0; k < 1000; k++)
        {
            ...
            // Process encoded symbols with degree one
            for (int i = 0; i < num_encoded; i++)
            {
                if (local_degree[i] == 1 && non_error[i])
                {
                    for (int j = 0; j < degree[i]; j++)
                    {
                        ...
                        if (sel != -1 && finished[sel] == 0)
                        {
                            // SIMD optimized XOR operation for reconstruction
                            int enc_idx = i * sym_size;
                            int rec_idx = sel * sym_size;

                            for (int l = 0; l < sym_size; l += 16)
                            {
                                __m128i enc_vec = _mm_loadu_si128((__m128i*)&encoded[enc_idx + l]);
                                _mm_storeu_si128((__m128i*)&reconstructed[rec_idx + l], enc_vec);
                            }
                ...
                if (non_error[i])
                {
                    for (int j = 0; j < degree[i]; j++)
                    {
                        ...
                        if (sel != -1 && finished[sel] == 1)
                        {
                            ...
                            {
                                __m128i enc_vec = _mm_loadu_si128((__m128i*)&encoded[enc_idx + l]);
                                __m128i rec_vec = _mm_loadu_si128((__m128i*)&reconstructed[rec_idx + l]);

                                __m128i result = _mm_xor_si128(enc_vec, rec_vec);
                                _mm_storeu_si128((__m128i*)&encoded[enc_idx + l], result);
                            }
