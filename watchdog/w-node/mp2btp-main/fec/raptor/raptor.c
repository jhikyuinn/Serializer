...
void** matrix2d_new( size_t esize, size_t ealign, size_t idim, size_t jdim ) {
	...
	//printf("alloc size: ptrs_size=%ld, element=%ld\n", ptrs_size, idim * row_size); fflush(stdout);
...
// LT encoder
char** LTEncoder(char *data, raptor_params_t *params)
{
	int K = params->K;
	// int H = params->H;
	// int S = params->S;
	...
	char **cc = MallocCharTwoArray(K, params->SYMBOL_SIZE);
	for (i = 0; i < params->K; i++)
		memcpy(cc[i], data + (i*params->SYMBOL_SIZE), params->SYMBOL_SIZE);
...
// LT decoder
char** LTDecoder(char **data, int dataMap[], raptor_params_t *params)
{
	int K = params->K;
	// int S = params->S;
	// int N = params->N;
