...
if [ "$1" == "clean" ]; then
    ...
    rm -f push_root push_child gmon.out peer_*block* *.txt
    cd ../pull
    ...
    make > /dev/null 2>&1
    cd ../example/push
    echo "Build example/push: go build push_root.go; go build push_child.go" 
    go build push_root.go; go build push_child.go   
    cd ../pull
